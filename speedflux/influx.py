...
class Influx:
        ...
        try:
            ...
            if (
                len(
                    list(
                        filter(
                            lambda x: x["name"] == self.config.INFLUX_DB_DATABASE,
                            databases,
                        )
                    )
                )
                ...
                self.client.create_database(self.config.INFLUX_DB_DATABASE)  # Create if
            else:
            ...
            if self.retries == 3:
                speedflux.LOG.error("Database Init failed for 3rd time. Exiting")
                sys.exit()
            self.retries += 1
            speedflux.LOG.error(
    ...
    def format_data(self, data):
        influx_data = [
            ...
            },
            {
                ...
                "time": data["timestamp"],
                "fields": {"packetLoss": int(data.get("packetLoss", 0))},
            },
            {
                "measurement": "speeds",
                "time": data["timestamp"],
    ...
    def write(self, data, data_type="Speedtest"):
        ...
        except (ConnectionError, NewConnectionError, Exception) as bad_connection:
            if self.retries == 3:
                speedflux.LOG.error(
